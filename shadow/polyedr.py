...
class Facet:
    ...
    def __repr__(self, vertexes):
        return f"{[i for i in self.vertexes]}"
        ...
        with open(file) as f:
            for i, line in enumerate(f):
                if i == 0:
                    ...
                    # массив стандартных вершин этой грани
                    lst = list(self.lst[int(n) - 1] for n in buf)
                    ...
                    # задание стандартной грани
                    self.lst_2.append(Facet(lst))

        self.sqr = 0
        cnt = 0
        for i in self.lst_2:
            if 1 < i.center().x**2 + i.center().y**2 + i.center().z**2 < 4\
                    and any(1 < n.x**2 + n.y**2 + n.z**2 < 4
                            for n in i.vertexes):0
                self.sqr += i.sqr()
            cnt += 1
